...
function(e) {
    ...
    M.prototype.renderData = function(e, n, o, r) {
        var c = this
          ...
          , sum = e[s.response.sum]
          , total = []
          , y = []
          , p = []
          , m = []
          , v = function() {
            !r && c.sortKey ? c.sort(c.sortKey.field, c.sortKey.sort, !0) : (layui.each(u, function(e, a) {
                var l = []
                  , o = []
                  , u = []
                  , h = e + s.limit * (n - 1) + 1;
                0 !== a.length && (r || (a[d.config.indexName] = e),
                c.eachCols(function(e, n, sum) {
                    var r = n.field || e
                      , f = a[r];
                    c.getColElem(c.layHeader, r);
                    if(n.total) total[r]=total[r]?(total[r]+(isNaN(Number(f))?0:Number(f))):(isNaN(Number(f))?0:Number(f));
            ...
            if(s.total){
            	...
            	c.eachCols(function(e, n) {
                    ...
                    if(typeof(total[r])!='undefined') td += '<div class="layui-table-cell laytable-cell-1-sales_num"><b><font color="red">' + eval(total[r]+'').toFixed(2) + '</font></b></div>'
                    td += '</td>';
                    totalTd.push(td);
                    var std = '<td>';
                    if(e==0)std = '<td style="text-align: center;">总计';
                    if(typeof(total[r])!='undefined') std += '<div class="layui-table-cell laytable-cell-1-sales_num"><b><font color="red">' + sum + '</font></b></div>'
                    std += '</td>';
                    sumTd.push(std);
            	});
            	var totalTr = c.layMain.find(".totalTr");
            	if(totalTr.length == 0){
            		totalTr = t('<tr class="totalTr">' + totalTd.join("") + '</tr><tr class="totalTr">' + sumTd.join("") + '</tr>');
